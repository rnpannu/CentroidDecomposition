...
public class GraphCanvas extends JComponent {
    ...
    public void paintComponent(Graphics g){
        ...
        for (Tree<NodeData, EdgeData>.Edge edge : tree.getEdge()) {
            g.setColor(edge.edgeData.getColor());
            Point s = edge.getHead().getData().getPosition();
            Point r = edge.getTail().getData().getPosition();
            doube dist = paintArrowLine(g, (int)s.getX(), (int)s.getY(), (int)r.getX(), (int)r.getY(), 20, 10, edge.getData().getDistance());
            if (edge.getData().getDistance()==-1) {
                edge.getData().setDistance(dist);
            }
        }
